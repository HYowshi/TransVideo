...
from videotrans.configure.config import ROOT_DIR, tr, logger, settings
    ...
    if os.path.exists(json_file):
        try:
            with open(json_file, 'r', encoding='utf-8-sig') as f:
                return DEFAULT_ASS_STYLE | json.load(f)
        except Exception as e:
            logger.exception(f"[set_ass_font] Cannot read or parse {json_file}: {e}", exc_info=True)

    if settings.get('subtitle_cover_original', True):
        logger.debug("[set_ass_font] ass.json not found; using built-in cover style for hard subtitles")
        return DEFAULT_ASS_STYLE.copy()

    logger.debug("[set_ass_font] ass.json not found and subtitle_cover_original is disabled")
    return {}
...
def set_ass_font(srtfile: str) -> str:
    ...
    default_style = (
        ...
        f"{style.get('Angle', 0)},"
        f"{style.get('BorderStyle', 3)},"
        f"{style.get('Outline', 6)},"
        f"{style.get('Shadow', 0)},"
        f"{style.get('Alignment', 2)},"
        f"{style.get('MarginL', 16)},"
        f"{style.get('MarginR', 16)},"
        f"{style.get('MarginV', 28)},"
        f"{style.get('Encoding', 1)}\n"
    )

    # 副样式：继承主样式，但 Fontsize 和 PrimaryColour 使用底部专用值
    bottom_fontsize = style.get('Bottom_Fontsize', 16)  # 默认 16
    ...
    bottom_backcolour = style.get('Bottom_BackColour', '&H55000000&')
...
def subtitle_display_width(text: str) -> float:
    """Weighted subtitle width inspired by VideoLingo's subtitle length rules."""
    width = 0.0
...
def _best_wrap_position(text: str, max_width: int, language: str) -> int:
    hard_pos = 0
    current_width = 0.0
    for idx, char in enumerate(text):
        current_width += subtitle_display_width(char)
        if current_width > max_width:
            break
        hard_pos = idx + 1

    if hard_pos >= len(text):
        return len(text)

    before_marks = "，,、；;：: "
    after_marks = "。！？.!?"
    search_back = max(0, hard_pos - (3 if language[:2] in contants.CJK_LANG else 12))
    search_forward = min(len(text), hard_pos + (2 if language[:2] in contants.CJK_LANG else 8))

    for idx in range(hard_pos, search_back, -1):
        if text[idx - 1] in after_marks:
            return idx
        if text[idx - 1] in before_marks:
            return idx

    for idx in range(hard_pos, search_forward):
        if text[idx] in after_marks:
            return idx + 1
        if text[idx] in before_marks:
            return idx + 1

    return max(1, hard_pos)
...
def simple_wrap(text: str, maxlen: int = 15, language: str = "en") -> str:
    ...
    max_width = max(3, int(maxlen))
    if subtitle_display_width(text) <= max_width + 4:
        return text
    ...
    while rest:
        rest = rest.strip()
        if subtitle_display_width(rest) <= max_width:
            lines.append(rest)
            break

        pos = _best_wrap_position(rest, max_width, language)
        line = rest[:pos].strip()
        if not line:
            line = rest[:max_width].strip()
            pos = max_width
        lines.append(line)
        rest = rest[pos:].strip()

    if len(lines) > 1 and subtitle_display_width(lines[-1]) < max_width / 3:
