...
from videotrans.configure.config import tr, params, settings, app_cfg, ROOT_DIR
...
class QuickDownloadWorker(QThread):
    ...
    def run(self):
        try:
            from videotrans.util.video_download import download_video

            path = download_video(self.url, progress=self.status.emit)
            self.done.emit(path)
...
class WinAction(WinActionBase):
    ...
    def _is_quick_source_ready(self, url: str) -> bool:
        return (
            bool(url)
            and getattr(self, "quick_source_url", "") == url
            and len(self.queue_mp4) == 1
            and Path(self.queue_mp4[0]).exists()
        )

    def _set_language_by_code(self, combo, *codes):
    ...
    def _start_quick_download(self, url: str):
        ...
        self.quick_downloader.status.connect(self.main.quick_status.setText)
        self.quick_downloader.done.connect(lambda path, source=url: self._quick_download_done(source, path))
        self.quick_downloader.failed.connect(self._quick_download_failed)
        self.quick_downloader.start()
        return True

    def _quick_download_done(self, url: str, path: str):
        self.quick_source_url = url
        ...
        if not self.main.target_dir:
            ...
            self.main.target_dir = quick_output.as_posix()
            self.main.output_dir.setText(tr('Translation results saved to:') + self.main.target_dir)
        self._apply_quick_defaults()
        self.main.startbtn.setDisabled(False)
        QTimer.singleShot(0, self.check_start)
    ...
    def _cleanup_quick_downloads(self):
        if not self.main.quick_delete_source.isChecked():
            return
        for file in getattr(self, "quick_downloaded_files", []):
            try:
                path = Path(file)
                if path.exists():
                    path.unlink()
    ...
    def check_start(self):
        ...
        if app_cfg.current_status == 'ing':
            ...
            return
        self.main.startbtn.setDisabled(True)
        quick_url = self._quick_url()
        if quick_url and not self._is_quick_source_ready(quick_url):
            self._start_quick_download(quick_url)
            return
        if quick_url:
            self._apply_quick_defaults()
